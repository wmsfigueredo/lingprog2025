...
function exe2() {
  ...
  alert(
    `Múltiplos de 2 ${multiplos2} Múltiplos de 3 ${multiplos3} e Múltiplos de 2 e de 3 ${multiplos2e3}`
  );
}
  ...
  do {
    ...
    for (let i = 0; i < 3; i++) {
      if (vetor[i].codigo == codigo) {
        // ENCONTRA O PRODUTO
        produtoEncontrado = vetor[i]; // GUARDA O PRODUTO
    ...
    cliente = Number(
      prompt(`Informe código do novo cliente. Digite 0 para encerrar`)
    );
  } while (cliente != 0); // fecha o do
  console.log(vetor);
}
...
function exe5() {
  ...
  for (let i = 0; i < 15; i++) {
    logica.push(
      Number(prompt(`Informe o ${i + 1}o. aluno que faz a disciplina Lógica`))
    );
  }
  for (let i = 0; i < 10; i++) {
    linguagem.push(
      Number(
        prompt(`Informe o ${i + 1}o. aluno que faz a disciplina Linguagem`)
      )
    );
  }
...
function exe6() {
  ...
  for (let i = 0; i < 3; i++) {
    ...
    vendas.push(
      Number(prompt(`Informe total vendido pelo ${i + 1}o vendedor`))
    );
    percentuais.push(
      Number(prompt(`Informe percentual de vendas do ${i + 1}o vendedor`))
    );
    // calcula a comissão
...
function exe9() {
  ...
  for (let i = 0; i < 10; i++) {
    let obejeto = {
      ...
      codigos: Number(prompt(`Informe código do produto ${i + 1}`)),
      precos: Number(prompt(`Informe preço do produto ${i + 1}`)),
    };
  }
  vetor.push(objeto);
...
function exe10() {
  ...
  for (let i = 0; i < 10; i++) {
    // para cada elemento de vetor 1
    if (vetor1[i] % 2 == 1) {
      // verifica se elemento é ímpar
      let qtdeDivisores = 0;
      for (let j = 0; j < 5; j++) {
        // para cada elemento de vetor 2
